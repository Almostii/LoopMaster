...
fn run_engine_test(capture_id: &str, render_id: &str, seconds: u64, allow_recovery: bool) -> ! {
    let graph = RouteGraph {
        ...
        buses: vec![BusSpec {
            id: BusId("mix".to_owned()),
            display_name: "mix".to_owned(),
        }],
        sinks: vec![SinkSpec {
        ...
        sends: vec![
            SendSpec::SourceToBus {
                id: SendId("capture-to-mix".to_owned()),
                source_id: SourceId("capture".to_owned()),
                bus_id: BusId("mix".to_owned()),
                gain_db: 0.0,
                muted: false,
                enabled: true,
                channel_map: Vec::new(),
            ...
            SendSpec::BusToSink {
                id: SendId("mix-to-render".to_owned()),
                bus_id: BusId("mix".to_owned()),
...
fn run_process_engine_test(pid: u32, render_id: &str, seconds: u64) -> ! {
    ...
    let graph = RouteGraph {
        ...
        sends: vec![
            SendSpec::SourceToBus {
                id: SendId("process-to-mix".to_owned()),
...
fn run_loopback_engine_test(loopback_render_id: &str, sink_render_id: &str, seconds: u64) -> ! {
    let graph = RouteGraph {
        ...
        sends: vec![
            SendSpec::SourceToBus {
                id: SendId("loopback-to-mix".to_owned()),
...
fn run_multi_sink_test(pid: u32, sink_a: &str, sink_b: &str, seconds: u64) -> ! {
    ...
    let graph = RouteGraph {
        ...
        sends: vec![
            ...
            SendSpec::BusToSink {
                id: SendId("mix-to-sink-a".to_owned()),
                bus_id: BusId("mix".to_owned()),
            ...
            SendSpec::BusToSink {
                id: SendId("mix-to-sink-b".to_owned()),
                bus_id: BusId("mix".to_owned()),
...
fn run_capture_render_test(
    ...
    let mut mixer = MixerPlan::new(&graph, 480, 2, 2).expect("空路由图混音计划有效");
