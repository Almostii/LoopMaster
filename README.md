<div align="center">
  ...
  <p>
    <img alt="正式版状态" src="https://img.shields.io/badge/状态-v1.2.1-2E8B57">
...
- **按进程路由软件声音**：可把某一个软件的播放声作为独立音频来源单独捕获——例如只取浏览器的网页音频、或只取游戏的声音，而系统其他程序、通知声不受影响。
...
- **内嵌 Web 控制台（局域网调音）**：在手机上用浏览器（无需安装 App）直接打开 `http://<电脑IP>:8920`，看到三列画布的实时镜像——推子、增益、静音、QR 扫码配对、可信设备管理全部在触屏上完成。HTTP 模式默认开放访问、HTTPS 模式可选（本地 CA + 服务器证书自动重签）。
- **网页端无线监听（WebRTC）**：浏览器一键监听任意输出通道（总线）的混音——Opus 96 kbps 立体声、20 ms 低延迟，经 WebRTC Audio Track 传输。监听状态（连接中/监听中/网络劣化/失败）一目了然，页面切后台回来自动恢复。
...
| 网页端无线监听 | 已支持 | 浏览器经 WebRTC（Opus 立体声）监听任意输出通道混音，推荐 Chrome/Edge（Firefox/Safari 需 HTTPS） |
...
| 无线监听 | Rust 2021（`app-service/web_server/monitor`：Opus 编码、webrtc-rs 推流、丢旧 FIFO） |
...
7. **无线监听（可选）**：网页控制台右上角点「监听」即可用手机听到该输出通道的混音（推荐 Chrome/Edge）；多条输出通道时可先下拉选择要监听的通道。首次使用请在桌面端设备页点「检查防火墙」放行监听媒体。
8. 部分拓扑变化（如重命名、通道映射）需要重启引擎后生效。
...
```text
dist-installer/
├── LoopMaster_1.2.1_x64-setup.exe    # NSIS 安装程序（推荐分发）
└── LoopMaster_1.2.1_x64_zh-CN.msi    # MSI 安装包
```
